...
error_enum Circular_Buffer_Add(Circilar_Buffer * CB, uint8_t d) {
	...
	if (!Circular_Buffer_Full(CB)) {         //Check if the buffer is not full
		...
		CB->Tail = CB->Tail % CB->capacity;     //Use Modulus operator to set the tail pointer to beginning if it
		...
		//critcial section ends here

		return Sucess;                   // success in allocating elements to buffer
